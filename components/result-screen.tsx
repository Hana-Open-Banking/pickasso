...
export default function ResultScreen() {
  ...
  const getSortedPlayers = () => {
    ...
    if (
      ...
      aiEvaluation.rankings.length > 0
    ) {
      console.log("🔍 Using AI rankings for sorting");
      // ✅ 원본 배열을 수정하지 않도록 복사본 생성
      const sortedRankings = [...aiEvaluation.rankings].sort(
        (a, b) => a.rank - b.rank
      );
      ...
      const sortedPlayers = sortedRankings
        .map((ranking) => {
          ...
          if (player) {
            ...
            console.error(`❌ Player not found for ranking:`, ranking);
            console.error(
              `❌ Available players:`,
              players.map((p) => ({ id: p.id, nickname: p.nickname }))
            );
            // 플레이어를 찾을 수 없는 경우 null 반환하여 필터에서 제거
            return null;
          }
        })
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2 text-white mb-4 text-sm">
            <div>
              플레이어: {players.length}명 | 점수데이터:{" "}
              {Object.keys(scores).length}개 | AI평가:{" "}
              {aiEvaluation ? "있음" : "없음"}
            </div>
        ...
        <div className="grid gap-6">
            ...
            <CardContent>
              <div className="space-y-4">
                {sortedPlayers.map((player, index) => {
                  const aiRanking = aiEvaluation?.rankings?.find(
                    (r) => r.playerId === player.id
                  );
                  const displayScore =
                    player.aiScore !== undefined
                      ? player.aiScore
                      : aiRanking
                      ? aiRanking.score
                      : scores[player.id] || 0;
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold ${getRankColor(
                              index,
                              player.id
                            )}`}
                          >
                ...
                <CardContent>
                  ...
                  {(() => {
                    ...
                    const myRanking = aiEvaluation.rankings?.find((r) => {
                      console.log(
                        `🎯 Comparing ${r.playerId} === ${currentPlayerId}:`,
                    ...
                    if (myRanking && myPlayer) {
                      ...
                      console.log(
                        "🎯 My comment section not rendered - data missing"
                      );
                      return null;
                    }
                  })()}
                  ...
                  <div className="space-y-4">
                    ...
                    {aiEvaluation.rankings
                      ?.filter(
                        (ranking) => ranking.playerId !== currentPlayerId
                      ) // 내 코멘트 제외
                      ...
                      .map((ranking, index) => {
                        ...
                        if (!player) {
                          console.warn(
                            `⚠️ No player found for ranking ${ranking.rank}`
                          );
                          return null;
                        }
          ...
          <div className="flex gap-4">
            ...
            >
              <Home className="h-4 w-4 mr-2" />방 나가기
            </Button>
          </div>
        </div>
      </div>
