...
export const useGameStore = create<GameState>((set, get) => {
  return {
    ...
    setAIEvaluation: (aiEvaluation) => {
      ...
      if (aiEvaluation) {
        const newAiEvaluation = {
          rankings: aiEvaluation.rankings
            ? [...aiEvaluation.rankings.map((r) => ({ ...r }))]
            : [],
          comments: aiEvaluation.comments
            ? [...aiEvaluation.comments.map((c) => ({ ...c }))]
            : [],
    ...
    createRoom: async () => {
      const state = get();
      // 🔥 기존 플레이어 ID가 있으면 재사용, 없으면 새로 생성
      const playerId = state.playerId || Date.now().toString();
        ...
        if (data.success && data.allSubmitted) {
          if (data.processing) {
            ...
            set({
              ...
              processingMessage:
                data.message || "AI가 작품을 평가하고 있습니다...",
            });
          } else {
